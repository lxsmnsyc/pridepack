...
const { argv } = yargs(process.argv.slice(2))
  ...
  .command(
    ...
    args =>
      args.positional('template', {
        describe: 'Project template to be used',
        default: 'basic',
        type: 'string',
      }),
  )
  .command('check', 'Performs typechecking')
  .command('watch', 'Opts-in to watch mode for building.')
  .command('clean', 'Cleans output directory.')
  .command('start', 'Runs the package in production mode.')
  .command('dev', 'Runs the package in development mode.')
  .demandCommand(1)
  .help();
